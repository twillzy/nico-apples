import React from 'react';

export default () => {
  return (
    <div className="case-study-overview porcelain-bg padding-top-sm padding-bottom-sm">
      <div className="margin-bottom-sm narrow-container ">
        <div className="narrow-container margin-bottom-sm">
          <h1>Feature Prioritisation</h1>
        </div>
        <h4 className="section-header">Defining Our MVP</h4>
        <p className="line-height-md">For our solution, we decided to focus only on our primary persona: Charlotte, the bar goer. We came up with a list of features that would benefit our primary persona and used them as a starting point for our design process.</p>
        <img className="margin-bottom-sm width-100 border-bottom-red" src="https://s3-ap-southeast-2.amazonaws.com/nicoapples-images/drinkr/features-board.jpg" alt="" />
        <img className="width-100 border-bottom-red" src="https://s3-ap-southeast-2.amazonaws.com/nicoapples-images/drinkr/features.png" alt="" />
      </div>
    </div>
  );
};
